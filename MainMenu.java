...
class MainMenu implements MouseListener, MouseMotionListener
{
	...
	public static boolean hardMode = false;		//hard mode makes all pieces white, removes ghosts, increases speed and acceleration, and awards double points
	...
	public void mouseClicked(MouseEvent e)
	{
		if(running)
		{
			...
			else if(e.getX() >= block * 8.5 && e.getX() <= block * 11.5 && e.getY() >= block * 23.5 && e.getY() <= block * 24.5)
			{
				hardMode = !hardMode;
			}
		}
	...
	public static void drawMusicMenus(Graphics g)
	{
		for(int q = 0; q < 3; q++)  //This loop draws three empty boxes
		{
			...
			g.fillRect(q * (block * 5) + block*7, (int)(block * 11.5), (int)(block * .5), block);		//BR corner of box
			...
			g.fillRect(q * (block * 5) + (int)(block*3.5), (int)(block * 9.5), (int)(block * 3.5), (int)(block * 2.5));
		}
		...
		switch(musicSelect)
		{
			...
			case 'B':
				...
				break;
			case 'C':
				g.fillRect(2 * (block * 5) + (int)(block*3.5), (int)(block * 9.5), (int)(block * 3.5), (int)(block * 2.5));
				break;
			default:
