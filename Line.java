...
public class Line extends Tetromino 
{
	...
	/*
	 *Rotation notation: 
	 *0 is 0123      3 is          2 is 3
	 *1 is   0            3210          2
	 *	     1                          1
	 *       2                          0
	 *       3
	 *The line rotates around a square in the middle and the 0 moves clockwise
	 */
...
/*Block[] temp = new Block[4];
		...
		if(rotation == 0) //if block starts at flat
		{
			if(dir == 1) //if rotating right, place the block at 3rd spot vert
			{
				temp[0] = new Block(blocks[2].blockX, blocks[2].blockY - 3);
				temp[1] = new Block(blocks[2].blockX, blocks[2].blockY - 2);
				temp[2] = new Block(blocks[2].blockX, blocks[2].blockY - 1);
				temp[3] = new Block(blocks[2].blockX, blocks[2].blockY);	
			}
			
			else if(dir == -1) //if ccw, place bottom at 2nd spot up
			{
				temp[0] = new Block(blocks[1].blockX, blocks[2].blockY - 3);
				temp[1] = new Block(blocks[1].blockX, blocks[2].blockY - 2);
				temp[2] = new Block(blocks[1].blockX, blocks[2].blockY - 1);
				temp[3] = new Block(blocks[1].blockX, blocks[2].blockY);
			}
			...
			for(Block t: temp)
			{
				...
				if(valid && !(Tetris.grid[t.blockY][t.blockX].equals(Color.BLACK) || Tetris.grid[t.blockY][t.blockX].equals(lineColor)))
				{
					valid = false;
				}
			}
		...
		else if(rotation == 1) //if the block starts vertical
		{
			if(dir == 1) //if rotating right, place bottom at 2nd
			{
				temp[0] = new Block(blocks[3].blockX-1, blocks[3].blockY);
				temp[1] = new Block(blocks[3].blockX, blocks[3].blockY);
				temp[2] = new Block(blocks[3].blockX+1, blocks[3].blockY);
				temp[3] = new Block(blocks[3].blockX+2, blocks[3].blockY);
			}
			
			else if(dir == -1) //if rotating left, place bottom at 3rd
			{
				temp[0] = new Block(blocks[3].blockX-2, blocks[3].blockY);
				temp[1] = new Block(blocks[3].blockX-1, blocks[3].blockY);
				temp[2] = new Block(blocks[3].blockX, blocks[3].blockY);
				temp[3] = new Block(blocks[3].blockX+1, blocks[3].blockY);
			}
