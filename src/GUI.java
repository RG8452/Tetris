...
public class GUI extends JFrame {
	...
	public static int block = EnterIntGUI("How many pixels do you want in each box? (Type either 32, 40, or 48)"); // Gets
																													// user
																													// input
																													// for
																													// screen
	...
	public static void main(String[] args) throws Exception {
		...
		case 'A':
			inputStream = AudioSystem.getAudioInputStream(new File("src/aTheme.wav"));
			clip.open(inputStream);
			clip.setLoopPoints(0, 1701850);
			break;
		case 'B':
			inputStream = AudioSystem.getAudioInputStream(new File("src/bTheme.wav"));
			clip.open(inputStream);
			clip.setLoopPoints(26915, 1586555);
			break;
		case 'C':
			inputStream = AudioSystem.getAudioInputStream(new File("src/cTheme.wav"));
			clip.open(inputStream);
		...
		while (true) {
			...
			if (MainMenu.musicSelect != 'M') {
				AudioInputStream as = AudioSystem.getAudioInputStream(new File("src/gameOver.wav")); // read in the game
																									// over sound
				clip.close();
	 ...
	 * myPanel.getGraphics() refers only to the screen The latter will be used more
	 * often to avoid math.
	 */
