...
class Tetris implements KeyListener, MouseListener
{
	...
	public static boolean firstHold = true;  //variable to check if a player is trying to hold a piece, happens only once
	...
	public static int delay = 1600;  //Delay time between piece movements down, in miliseconds. Default is 1.5 seconds (1500 miliseconds)
...
//		System.out.println("You typed " + c);
	...
	{
		if((System.nanoTime()/1000000) - start >= delay)	//If the current time is [delay] seconds after the last recorded time
	    {
...
/*		for(Block bbb: curTetromino.blocks)
		{
			if(bbb.blockY <= row)
			{
				bbb.blockY++;
	...
	{	
		...
		timerStart = start;
		placeTimer = start;
		delay = 1600;  //Delay time between piece movements down, in miliseconds. Default is 1.5 seconds (1500 miliseconds)
	...
	public static void drawNextPieces(Graphics g)  //fills in the NEXT menu
	{
		...
		for(int z=0; z<3; z++) //loop through each piece
		{
			for(Block b: nextTetrominos.get(z).blocks)
			{
				...
				g.fillRect((int)(sBlock * guideX) + sPixel, sBlock * (guideY+1+3*z) - sPixel, sPixel * 7, sPixel);
				g.fillRect((int)(sBlock * (guideX+1)) - sPixel, sBlock * (guideY+3*z) + sPixel, sPixel, sPixel * 7);

				g.setColor(curColor.darker().darker());
				g.fillRect((int)(sBlock * (guideX+1)) - sPixel, sBlock * (guideY+1+3*z)-sPixel, sPixel, sPixel);
			}
		}
	}
	...
	public static void drawHeldPiece(Graphics g)  //fills in the NEXT menu
	{
		...
		if(heldTetromino != null)
		{
			for(Block b: heldTetromino.blocks)
			{
				...
				g.setColor(curColor.darker());
				g.fillRect((int)(sBlock * guideX) + sPixel, (int)(sBlock * (guideY+1)) - sPixel, sPixel * 7, sPixel);
				g.fillRect((int)(sBlock * (guideX+1)) - sPixel, (int)(sBlock * guideY) + sPixel, sPixel, sPixel * 7);

				g.setColor(curColor.darker().darker());
				g.fillRect((int)(sBlock * (guideX+1)) - sPixel, (int)(sBlock * (guideY+1))-sPixel, sPixel, sPixel);
			}
		}
	}
		...
		if(block % 3 == 0)
		{
			...
			for(Tetromino tet: allBlocks)
			{
				for(Block b: tet.blocks)
				{
					...
					g.setColor(curColor.darker());
					g.fillRect((int)(tBlock * guideX) + tPixel, (int)(tBlock * (guideY+1)) - tPixel, tPixel * 7, tPixel);
					g.fillRect((int)(tBlock * (guideX+1)) - tPixel, (int)(tBlock * guideY) + tPixel, tPixel, tPixel * 7);

					g.setColor(curColor.darker().darker());
					g.fillRect((int)(tBlock * (guideX+1)) - tPixel, (int)(tBlock * (guideY+1))-tPixel, tPixel, tPixel);
				}
				yShift += .25 * tBlock;
			}
		}
		...
		else if(block == 32)
		{
			...
			for(Tetromino tet: allBlocks)
			{
				for(Block b: tet.blocks)
				{
					...
					g.setColor(curColor.darker());
					g.fillRect((int)(tBlock * guideX) + tPixel, (int)(tBlock * (guideY+1)) - tPixel, tPixel * 7 + 2, tPixel);
					g.fillRect((int)(tBlock * (guideX+1)) - tPixel, (int)(tBlock * guideY) + tPixel, tPixel, tPixel * 7 + 2);
	...
	public static void drawMenus(Graphics g)  //draws each menu
	{
		...
		g.drawString(timeOut.format(seconds / 60) + ":" + timeOut.format(seconds%60), (int)(block * 17), (int)(block * 21.5 - block/3));
